...
function addFilesCommitTagPush(cb, files = null) {
    if (!files || !Array.isArray(files)) {
        return Promise.reject(
            "Files property is required to be an array of files to 'git add'.",
        );
    }
...
function tagAndPush(files = ['package.json'], bump = 'patch') {
    if (bump === 'major') {
        return gulp.series('bump:major', cb => {
            addFilesCommitTagPush(cb, files);
        });
    } else if (bump === 'minor') {
        return gulp.series('bump:minor', cb => {
            addFilesCommitTagPush(cb, files);
        });
    }
    return gulp.series('bump:patch', cb => {
        addFilesCommitTagPush(cb, files);
    });
}
...
gulp.task(
    'buildAndReleasePatch',
    gulp.series('clean', 'build', 'release:patch'),
);
gulp.task(
    'buildAndReleaseMinor',
    gulp.series('clean', 'build', 'release:minor'),
);
gulp.task(
    'buildAndReleaseMajor',
    gulp.series('clean', 'build', 'release:major'),
);
